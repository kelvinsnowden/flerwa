...
| SYS-O3 | Dedicated scheduler/jobs history view (not just "last run") | **Implemented this pass** — new `/admin/system`: full run history, filterable by job name, paginated, duration computed, result payload expandable. Pure read (RLS `admin can read scheduler runs`, confirmed no write policy for any client role — only the service-role cron routes insert). Dashboard's auto-approve-sweep card now links here. **Honest limitation:** `scheduler_runs` has 0 rows in production (no cron has actually run yet — same Vercel-project-access gap noted throughout this program), so only the empty state was exercised, not the populated table render | `/admin/system` (new) | `scheduler_runs` table | Admin | Low (read) | N/A | N/A | — | — |
