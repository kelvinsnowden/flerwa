...
## 14. Implementation status ("Make the Marketplace Production-Scale Ready", follow-on task)

Per-finding status after the follow-on implementation pass. See `MARKETPLACE_SCALE_IMPLEMENTATION_PLAN.md` for the full scoping rationale (what was selected vs. deferred and why) and the git log on this branch for the exact commits. Every migration below was applied directly to the live project (`famdxoardiibonghxepl`) via the Supabase MCP tool and also committed as a plain-SQL file under `supabase/migrations/`; nothing was pushed to the remote branch without explicit instruction, and no production data (rows) was modified — only schema (new tables/columns/indexes/functions) and application code changed.

| §4/§11 finding | Status | What changed | Verification performed |
|---|---|---|---|
| No scheduler ever invoked `rpc_run_auto_approve_sweep` (§4 #1, P0) | **Fixed** | `vercel.json` Cron entry (daily 03:00 UTC) → `src/app/api/cron/auto-approve-sweep/route.ts` (fails closed without `CRON_SECRET`) → `rpc_run_auto_approve_sweep_locked()` (advisory-lock overlap guard around the original, untouched function) → `scheduler_runs` table, surfaced on `/admin`. | `tsc`/build clean. Overlap-guard logic reviewed against the function body; true cross-session lock contention was **not** independently verified (single-connection tool limitation, stated honestly rather than claimed). Real schedule firing depends on the Vercel deploy actually registering the cron job and `CRON_SECRET` being set in the dashboard — not verifiable from this session. |
| 7 unindexed foreign keys (§4 #4) | **Fixed** | 7 `create index` statements, one per flagged FK. | Confirmed via a fresh Supabase advisor run that all 7 no longer appear in `unindexed_foreign_keys`. |
| `auth_rls_initplan` (33 findings) / `multiple_permissive_policies` (105 findings) (§5, P2) | **Partially fixed** | `is_admin()`, `is_txn_participant()`, `is_conversation_participant()` wrapped `auth.uid()` as `(select auth.uid())` (benefits every policy calling them). `messages`' 6 policies consolidated to 3. | Confirmed via a fresh advisor run that `messages` no longer appears in either finding at all. The other ~25 tables' own policies are unchanged — deferred, see plan doc; still an open P2 item. |
| Unbounded `messages` inbox/thread queries (§8, implicit) | **Fixed** | New `last_message_at` column (trigger-maintained) on `service_transactions`/`conversations`; inbox now queries threads (bounded, paginated, `?limit=` capped at 400) with one embedded latest message each instead of full message history; both thread views load the most recent 60 messages with a "Load earlier" cursor control. | Live role-simulated SQL: participant sees exactly their thread with correct latest message + unread count; non-participant sees none; trigger is monotonic under out-of-order inserts. `tsc`/build clean. |
| No idempotency on payment webhook ingestion (§6, implicit) | **Fixed** | `rpc_ingest_payment_event` dedupes on `(provider_key, external_reference, event_type)` via a partial unique index; a retried delivery returns the original event id instead of recording a duplicate or re-funding. | Live: two identical deliveries → 1 event row, 1 funding; a differently-typed event with the same reference is still recorded and acted on; two unparseable (empty-reference) payloads never collide. |
| No idempotency on request-mode booking (§6, implicit) | **Fixed** | `rpc_book_service` accepts an optional per-customer idempotency key (client generates one per form mount); a second call with the same key returns the original transaction id. **Self-caught regression**: adding the parameter via `CREATE OR REPLACE` created a second function overload with an unintended `anon`-execute grant — caught via a live `pg_proc` privilege check within the same session and corrected in the very next migration (old overload dropped, correct grants restored on the survivor). | Live: same key → 1 transaction; distinct/omitted keys → separate transactions as before. `pg_proc` re-checked after the fix: exactly one overload, `authenticated`-only. |
| No caching anywhere (§8, implicit) | **Partially fixed** | `src/lib/cache/catalog.ts`: `unstable_cache`-wrapped reads for the home page's active-categories list and recommended-services list only (60s revalidate), via a plain anon-key client (no cookies, since `unstable_cache` can't depend on request-scoped state). No full-page or auth-sensitive caching added anywhere. | `tsc`/build clean. The success path (cache populated against a real, reachable database) could not be exercised live in this sandbox — its own network-egress policy blocks direct outbound Supabase access from local dev-server processes (confirmed via the `/api/health` test below), so this is inspected/reviewed, not live-tested end-to-end. |
| No rate limiting anywhere (§9, implicit) | **Fixed for 5 named actions** | Postgres-backed fixed-window limiter (`rate_limit_buckets` + `rpc_check_rate_limit`, self-cleaning, no Redis) wired into `bookService`, `submitQuote`, `sendMessage`, `postTask`, `signup`. Keyed server-side by `auth.uid()` (never a caller-supplied identity) except the necessarily-IP-keyed pre-auth signup path. Fails open on an infra error. | Live: a 2-per-window limit correctly rejects the 3rd call; an authenticated caller cannot target another user's bucket by supplying an arbitrary identity string (ignored whenever `auth.uid()` is set). Other server actions (admin actions, profile edits, availability management, etc.) were **not** rate limited — out of the plan's named scope, not evaluated this pass. |
| No health check endpoint (§12, implicit) | **Fixed** | `GET /api/health` — no auth required, checks real DB connectivity via a public-RLS-safe read against `categories`, returns 200/`ok:true` or 503/`ok:false`, `Cache-Control: no-store`. | Locally, correctly returned 503 + the real underlying error when the sandbox's own network policy blocked outbound Supabase access — proves the failure path works. The 200 success path is exercised implicitly by every other page using the same client successfully; not independently curled against a reachable database from this session. |
| Real load test (§10) | **Not performed** | — | No safe, isolated (staging/branch) environment was available this session to load-test against; running one against the live project would risk its rows and violate this task's own safety rules. Explicitly not claimed as done — see rule 8/9 of the original task. |
| Infra plan-limit confirmation (§9) | **Not performed** | — | Requires Supabase/Vercel dashboard access this session does not have; still an open unknown, unchanged from §9/§13. |

### Deferred this pass, unchanged from the original audit
RLS optimization for the ~25 tables beyond `messages` and the three shared predicate functions; background jobs beyond the one scheduler route; a real error-tracking/observability vendor integration; a staging Supabase project; the full load-test scenario suite in §10. See `MARKETPLACE_SCALE_IMPLEMENTATION_PLAN.md` for why each was scoped out rather than attempted shallowly.
