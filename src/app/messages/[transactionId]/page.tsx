...
export default async function MessageThreadPage({
  ...
  const { data: txn, error: txnError } = await supabase
    ...
    .select("id, customer_id, services(name), providers(display_name, user_id), profiles:customer_id(full_name)")
    .eq("id", transactionId)
    .single<{
      id: string;
      customer_id: string;
  ...
  const otherName = isCustomer ? txn.providers?.display_name ?? "Provider" : txn.profiles?.full_name ?? "Customer";
