...
export default async function SavedProvidersPage() {
  ...
  return (
    <div className="mx-auto max-w-2xl px-4 py-8 pb-4">
      <h1 className="text-2xl font-bold mb-6">Saved professionals</h1>

      {error && (
        <ErrorNotice message="We couldn't load your saved professionals right now. Please refresh — this does not mean you have none saved." />
      )}
      ...
      {!error && !saved?.length && (
        <EmptyState
          ...
          body="Tap the heart on a professional's profile to keep them here for later."
          action={
